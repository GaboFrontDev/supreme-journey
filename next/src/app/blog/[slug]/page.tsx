...
export async function generateStaticParams(): Promise<{ slug: string, attributes: PageAttributes }[]> {
  const res = await fetchWithToken(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/blogs?${qs.stringify(queryParams)}`);
...
export default async function Page({ params }: { params: { slug: string }}) {
  const res = await fetchWithToken(`${process.env.NEXT_PUBLIC_STRAPI_API_URL}/blogs?filters[slug][$eq]=${params.slug}&${qs.stringify(queryParams)}`);
  const data = await res.json() as StrapiResponse;
      ...
      <Section width='max-w-7xl' paddingTop='pt-12'>
        <div className='flex items-start justify-between gap-36'>
          <div>
            ...
            <span className='text-lg font-bold text-[#A1A1A1]'>{formatDateToSpanish(attributes.createdAt)}</span>
          </div>
        </div>
      </Section>
      {sections.map((section, index) => (
        <SectionRenderer key={`${section.__component}-${index}`} component={section} />
      ))}
      <Footer />
    </main>
  );
