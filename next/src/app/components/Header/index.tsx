...
export default function Header({
  ...
  return (
    <>
      {/* desktop */}
      <motion.div
        className={`left-1/2 top-6 z-50 hidden w-full max-w-7xl -translate-x-1/2 rounded-full px-6
        py-3 shadow-lg backdrop-blur-sm md:fixed md:block ${backgroundColorClass}`}
        ...
        <div className='flex items-center justify-between'>
          ...
          >
            <div className='flex h-6 w-6 flex-col items-center justify-center'>
              <span className={`${hamburgerColorClass} h-0.5 w-5`}></span>
              <span className={`${hamburgerColorClass} mt-1 h-0.5 w-5`}></span>
              <span className={`${hamburgerColorClass} mt-1 h-0.5 w-5`}></span>
            </div>
          </button>
        </div>
      ...
      {isMobileMenuOpen && (
        <motion.div
          className='fixed inset-0 z-40 select-none bg-black/60 backdrop-blur-sm'
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
